...
class Lexer:
    ...
    TOKEN_REGEXES = [(token_type, re.compile(pattern)) 
    ...
    def tokenize(self):
        ...
        while self.position < len(self.text):
                ...
                if match:
                    ...
                    if token_type != 'WHITESPACE':
                        ...
                        if token_type == 'STRING':
                            ...
                            if value.startswith('"'):
                                actual_value = value[1:-1].replace('\\"', '"').replace('\\\\', '\\')
                            else:
                                actual_value = value[1:-1].replace("\\'", "'").replace('\\\\', '\\')
                            token = Token(token_type, {'raw': value, 'value': actual_value}, self.position)
                        else:
            ...
            if not matched:
                ...
                raise CELSyntaxError(f"Unexpected character: '{char}'", 
