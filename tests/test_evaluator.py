...
from pycel.evaluator import evaluate
...
class TestEvaluator(unittest.TestCase):
    ...
    def test_member_access_evaluation(self):
        ...
        result = evaluate(parse("obj.unknown"), context, allow_undeclared_vars=True)
        self.assertIsNone(result)
        
        # Undefined field with allow_undeclared_vars=False
        with self.assertRaises(CELUndefinedError):
            evaluate(parse("obj.unknown"), context, allow_undeclared_vars=False)
        
        # Access on null with allow_undeclared_vars=True
        result = evaluate(parse("null.field"), context, allow_undeclared_vars=True)
        self.assertIsNone(result)
    ...
    def test_index_access_evaluation(self):
        ...
        result = evaluate(parse("arr[10]"), context, allow_undeclared_vars=True)
        self.assertIsNone(result)
    ...
    def test_binary_logical_operations(self):
        ...
        # Short-circuit AND
        result = evaluate(parse("f && unknown"), context, allow_undeclared_vars=True)
        ...
        # Short-circuit OR
        result = evaluate(parse("t || unknown"), context, allow_undeclared_vars=True)
    ...
    def test_ternary_operation(self):
        ...
        # Nested ternary
        result = evaluate(parse("x < y ? (x == 0 ? 'zero' : 'positive') : 'negative'"), context)
        self.assertEqual(result, 'positive')
        
        # Short-circuit evaluation
        result = evaluate(parse("true ? x : unknown"), context, allow_undeclared_vars=True)
